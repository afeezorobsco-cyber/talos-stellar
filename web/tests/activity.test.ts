...
describe("GET /api/activity", () => {
  ...
  it("returns an opaque cursor and traverses pages without gaps or duplicates", async () => {
    ...
    const secondResponse = await GET(
      new Request(
        `http://localhost/api/activity?limit=2&cursor=${firstBody.nextCursor}`,
      ),
    );
    const secondBody = await secondResponse.json();
    const ids = [...firstBody.transactions, ...secondBody.transactions].map(
      (item) => item.id,
    );

    expect(fetchActivityTransactions).toHaveBeenNthCalledWith(1, 2, null);
    expect(fetchActivityTransactions).toHaveBeenNthCalledWith(
      2,
      2,
      pages[0].nextCursor,
    );
  ...
  it("returns 400 for a malformed cursor", async () => {
    const response = await GET(
      new Request("http://localhost/api/activity?cursor=not-a-cursor"),
    );
  ...
  it("traverses deterministic fixture pages without gaps, duplicates, or null cursor regressions", async () => {
    const fixturePages = [
      {
        transactions: [
          {
    ...
    fetchActivityTransactions
      .mockResolvedValueOnce(fixturePages[0])
      .mockResolvedValueOnce(fixturePages[1])
      .mockResolvedValueOnce(fixturePages[2]);
    ...
    const pageTwo = await GET(
      new Request(
        `http://localhost/api/activity?limit=2&cursor=${pageOneBody.nextCursor}`,
      ),
    );
    const pageTwoBody = await pageTwo.json();
    const pageThree = await GET(
      new Request(
        `http://localhost/api/activity?limit=2&cursor=${pageTwoBody.nextCursor}`,
      ),
    );
    const pageThreeBody = await pageThree.json();

    const combined = [
      ...pageOneBody.transactions,
      ...pageTwoBody.transactions,
      ...pageThreeBody.transactions,
    ];
